...
test.describe('Journey 08: LLM Provider & Model Selection', () => {
  ...
  test('save config fires saveConfig action', async ({ page }) => {
    ...
    // Try to trigger a change first — type in a search/filter input if visible
    ...
    if (await saveBtn.first().isVisible({ timeout: 10000 }).catch(() => false)) {
      if (await saveBtn.first().isEnabled({ timeout: 2000 }).catch(() => false)) {
        ...
        // No changes detected — skip (save button disabled because config matches session defaults)
        test.skip(true, 'Save button disabled — no unsaved changes detected in test environment');
      }
    } else {
