...
export function ClipsDock({
  ...
  return (
    <>
      {/* ── Bottom bar ─────────────────────────────────────────── */}
      <div className="fixed bottom-0 left-0 right-0 z-40 flex items-center gap-3 h-14 px-4 bg-white/85 backdrop-blur-md border-t border-border/50 shadow-lg">

        {/* Label */}
        {clips.length === 0 ? (
          <span className="text-[11px] text-muted/60 italic shrink-0">
            No clips yet — highlight &amp; clip articles from the feed
          </span>
        ) : (
          <span className="text-[11px] font-semibold text-muted/70 uppercase tracking-wider shrink-0">
            Clips
          </span>
        )}
        ...
        <div className="flex-1 flex items-center gap-1.5 overflow-x-auto min-w-0" style={{ scrollbarWidth: 'none' }}>
          {visibleClips.map((clip, i) => (
            ...
            >
              <div
                className={[
                  'w-9 h-9 rounded-lg overflow-hidden border border-white/60 shadow-sm transition-transform duration-150',
                  hoveredIndex === i ? 'scale-125 shadow-md' : '',
                ].join(' ')}
              >
                {clip.thumbnailUrl ? (
                  <img src={clip.thumbnailUrl} alt="" className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full bg-gradient-to-br from-primary/30 to-violet-400/50" />
                )}
              </div>
          ...
          {extraCount > 0 && (
            <button
              ...
              className="shrink-0 h-9 px-2.5 rounded-lg bg-white/60 border border-border/50 text-[10px] font-semibold text-muted hover:text-ink hover:bg-white/80 transition-colors"
            >
              +{extraCount}
            </button>
          )}
        ...
        <button
          type="button"
          onClick={() => setExpanded(true)}
          className="shrink-0 flex items-center gap-1.5 h-8 px-3 rounded-lg border border-border/50 bg-white/60 text-xs font-medium text-muted hover:text-ink hover:bg-white/80 transition-colors"
        >
          <ChevronUp size={13} />
          {clips.length > 0 ? `${clips.length} saved` : 'Clips'}
        </button>
      </div>
