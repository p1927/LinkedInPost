/**
 * Journey 23: Wiring Loop 3/50 — Setup Wizard Wiring Validation
 *
 ...
 *   - validate-cloudflare POST should return correct {ok, result} shape
 *   - high-progress state (>=90%) should show status dashboard
 *   - wizard renders welcome step without crash
 *   - write-config failure is handled gracefully
 *   - unknown API endpoints return JSON 404 (not HTML)
 *   - deployment-mode POST body includes `mode` field
 *
 * References:
...
// ---------------------------------------------------------------------------
// Journey 23.1: Wizard — Initial Load & Welcome Step
// ---------------------------------------------------------------------------

test.describe('Journey 23.1: Wizard — Initial Load', () => {
  ...
  test('wizard renders without crash on initial load', async ({ page }) => {
    ...
    // Wait for wizard to load and make API calls — also wait for the
    // deployment-mode radio buttons to appear so the test is deterministic.
    // The wizard has an async state-detection chain (project-path → state)
    // that must resolve before the deployment-mode step renders.
  ...
  test('wizard shows welcome step or dashboard on fresh state load', async ({ page }) => {
    await setupSetupApiMocks(page, {
  ...
  test('wizard page loads with project-path and state API calls', async ({ page }) => {
    ...
    // Wizard should have called project-path and/or state — assert wiring is live
...
// ---------------------------------------------------------------------------
// Journey 23.2: Wizard — State Management (high-progress dashboard)
// ---------------------------------------------------------------------------

test.describe('Journey 23.2: Wizard — State Management', () => {

  test('high-progress state (90%) shows status dashboard not wizard steps', async ({ page }) => {
    /**
     * Spec: When setup is near-complete (high progress), the wizard should show
     * the status dashboard (progress percentage) rather than re-running wizard steps.
     *
...
// ---------------------------------------------------------------------------
// Journey 23.3: Wizard — API Connection Wiring
// ---------------------------------------------------------------------------

test.describe('Journey 23.3: Wizard — API Connection Wiring', () => {
  ...
  test('deployment-mode radio selection persists on Continue click', async ({ page }) => {
    const mocks = await setupSetupApiMocks(page);
    await page.waitForLoadState('domcontentloaded');
    await gotoWizard(page);
...
// ---------------------------------------------------------------------------
// Journey 23.4: Wizard — Error Handling & Resilience
// ---------------------------------------------------------------------------

test.describe('Journey 23.4: Wizard — Error Handling', () => {
...
// ---------------------------------------------------------------------------
// Journey 23.5: Wizard — API Response Shape Validation
// ---------------------------------------------------------------------------

test.describe('Journey 23.5: Wizard — API Response Shape', () => {

  test('validate-cloudflare mock returns ok:true with result on success', async ({ page }) => {
    /**
     * Spec: POST /api/setup/validate-cloudflare with valid token
     * should return { ok: true, result: { account: { id: '...' } } }.
     *
     * Expected behavior: Mock returns 200 with { ok: true, result: {...} }.
     *
     * Bug: Returns 400 (Cloudflare validation fails even with mock token).
     *
     * Note: page.evaluate (browser fetch) ensures Playwright route handlers intercept.
     */
    await setupSetupApiMocks(page, {
    ...
    // This ensures Playwright route handlers are active — page.request bypasses them.
  ...
  test('validate-cloudflare mock returns ok:false with error on failure', async ({ page }) => {
    await setupSetupApiMocks(page, {
  ...
  test('unknown API endpoint returns JSON 404 (not HTML)', async ({ page }) => {
     ...
     *
     * Bug: Returns HTML (possibly from Express catch-all or static file server).
     * The HTML response causes JSON parse failures in the wizard.
     */
    await setupSetupApiMocks(page);
    await page.waitForLoadState('domcontentloaded');
    await gotoWizard(page);
    ...
    // Response should be JSON (not HTML) — avoids SyntaxError on parse
...
// ---------------------------------------------------------------------------
// Journey 23.6: Wizard — write-config Wiring
// ---------------------------------------------------------------------------

test.describe('Journey 23.6: Wizard — write-config Wiring', () => {
...
// ---------------------------------------------------------------------------
// Journey 23.7: Settings Page — Bootstrap Wiring
// ---------------------------------------------------------------------------

test.describe('Journey 23.7: Settings Page — Bootstrap Wiring', () => {
